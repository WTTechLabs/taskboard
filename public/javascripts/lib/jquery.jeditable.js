		...
		types: {
			...
			textselect: {
				element : function(settings, original) {
					var div = $('<div/>');
					var input = $('<input>');
					if (settings.width  != 'none') { input.width(settings.width - 400);  }
					/* https://bugzilla.mozilla.org/show_bug.cgi?id=236791 */
					//input[0].setAttribute('autocomplete','off');
					input.attr('autocomplete','off');
					div.append(input).append(' for ').append('<select name="updated_at"><option value="yesterday">yesterday</option><option value="today" selected="selected">today</option><option value="tomorrow">tomorrow</option></select>');
					$(this).append(div);
					return(input);
				}
			},
